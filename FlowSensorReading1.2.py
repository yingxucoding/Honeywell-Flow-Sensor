...
ser = serial.Serial()
ser.baudrate = 9600        # define it based on the property of the device connected to the COM port
ser.port = ask_for_port()
print ("Opening port")
ser.close()     # if the serial port is already open, close it.
ser.open()
...
#    scheduler.add_job(flush, 'interval', seconds = 300)
    scheduler.add_job(flush, trigger)       # flush buffer every several seconds
    scheduler.start()
